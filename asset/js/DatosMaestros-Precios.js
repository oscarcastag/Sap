...
$(document).ready(function(){
    ...
    $.ajax({
        type: "GET",
        url: "BAckEndSAP/Precios.php",
        data: "action=Ex",
        success: function (text) {
            //console.log(text);
            console.log(text.data.EtPricesucData);
            var date = text.data.EtPricesucData.item;
            var txt = "";
            console.log(date);
            txt += '<div class="table-responsive"> <table id="3x" class="table table-striped table-bordered table-hover">';
            txt += '<thead class="table-primary"> <tr><th>Clave</th> <th>Fabrica</th> <th>Almacen</th><th>CLABS</th><th>VORMG</th><th>Vkbur</th></tr> </thead>';
            for (x in date) {
                txt += "<tr><td>" + date[x].Matnr + "</td>";
                txt += "<td>" + date[x].Maktg + "</td>";
                txt += "<td>" + date[x].Knumh + "</td>";
                txt += "<td>" + date[x].Kbetr + "</td>";
                txt += "<td>" + date[x].Konwa + "</td>";
                txt += "<td>" + date[x].Vkbur + "</td></tr>";
            }
            txt += "</table> </div>"
